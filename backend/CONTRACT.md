...
### POST /clients  (он же POST /api/clients)
Открыть счёт новому клиенту (онбординг) — так растёт клиентская база.
Создаёт запись клиента, начисляет приветственный бонус в кешбэк и, если указан
`referred_by`, записывает реферал и платит бонус обеим сторонам. Принимает JSON
`{name, segment?, income_rub?, balance_rub?, age?, products?, referred_by?,
welcome_bonus_rub?, referral_bonus_rub?}`:
- `name` — имя клиента, обязателен;
- `segment` — по умолчанию `mass`; `products` — по умолчанию `["debit"]`;
- `referred_by` — id пригласившего клиента (необязательно); если он не найден,
  счёт всё равно открывается, но реферальный бонус не начисляется;
- `welcome_bonus_rub` — приветственный бонус (по умолчанию 1000);
- `referral_bonus_rub` — бонус по рефералу обеим сторонам (по умолчанию 1000).

Возвращает `{status, client, welcome_bonus_rub, referral}`, где `client` —
созданный объект клиента (новый `id` вида `c-XXXXX`), `referral` — запись
реферала или `null`. `400`, если не указано имя.
...
### POST /clients/{client_id}/credit-cashback  (он же /api/clients/{id}/credit-cashback)
Начислить кешбэк на счёт клиента — общий примитив (им retail платит
реферальный бонус, промо и т.п.). Принимает JSON `{amount_rub, reason?}`.
Возвращает `{status, client_id, credited_rub, cashback_balance_rub, tx_id, ts}`.
Создаёт транзакцию `cashback_credit` (`cashback_rub` = начислено). `404`, если
клиента нет; `400`, если сумма не положительная.

## Реферальная программа

Кто кого привёл; бонус начисляется кешбэком обеим сторонам. Один приглашённый
может быть привязан только к одному пригласившему.

### POST /referrals  (он же POST /api/referrals)
Записать связь «пригласивший → приглашённый» и, если задан `bonus_rub`,
начислить бонус кешбэком обеим сторонам. Принимает JSON
`{inviter_id, invitee_id, bonus_rub?}`. Если `bonus_rub` не передан или 0 —
связь записывается со статусом `pending` (бонус можно выплатить позже через
`credit-cashback`). Возвращает `{status, referral, inviter_cashback_rub,
invitee_cashback_rub}`, где `referral` —
`{referral_id, inviter_id, invitee_id, bonus_rub, status (paid|pending),
created_at}`. `404` — если кто-то из клиентов не найден; `400` —
самоприглашение или приглашённый уже привязан к другому пригласившему.

### GET /clients/{client_id}/referrals
Рефералы клиента: кого он привёл и кто привёл его. Возвращает
...
status, opened_at, matures_at, accrued_interest_rub}`. `matures_at` = дата
открытия + `term_months`. `accrued_interest_rub` — накопленные проценты
(растут при вызове accrue-interest).
...
Если по вкладу уже накоплены проценты (`accrued_interest_rub` > 0), они берутся
как база процентов вместо расчёта за весь срок.

### POST /deposits/accrue-interest  (он же /api/deposits/accrue-interest)
Начислить проценты по всем активным вкладам за `days` дней (по умолчанию 30) —
так деньги клиента видимо растут. Принимает `{days?}`. Возвращает
...
Один вклад по id вклада (`d-...`). Если передать id клиента (`c-...`) — вернёт
список его вкладов `{client_id, total, items}` (этот путь зовёт retail как
`GET /deposits/{client_id}`). `404`, если ничего не найдено.

## Алиасы под retail

Retail зовёт backend по этим путям — отдаём реальные данные (раньше он падал
в симуляцию).

### GET /portfolio/{client_id}
То же, что `GET /clients/{client_id}/portfolio` (портфель клиента).

### GET /credit-card/{client_id}
Сводка по основной (первой активной) карте клиента. Возвращает
